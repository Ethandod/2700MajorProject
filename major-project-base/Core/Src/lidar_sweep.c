...
void omni_sweep(uint8_t direction, uint16_t *previousReadings, uint16_t *output)
{
	...
	while(reading_count < READINGS_PER_SWEEP - 1)
	{
		set_pan(current_pwm);
		...
		if (previousReadings != NULL)
		{
			delta = abs(output[reading_array_position]-previousReadings[alt_reading_position]);

			sprintf(string_to_send, "[%hu] : {%hu vs %hu} --> [%hu]\r\n",reading_array_position, output[reading_array_position], previousReadings[alt_reading_position], delta);
			SerialOutputString(string_to_send, &USART1_PORT);
